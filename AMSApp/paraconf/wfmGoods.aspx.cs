...
//				this.Button3.Enabled=false;
		...
		protected void Button1_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				DataTable dtout=m1.GetGoods(strGoodsID,strGoodsName);
				if(dtout==null)
				{
		...
		protected void Button3_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				DataTable dtout=m1.GetPackageInfo(strGoodsID,strGoodsName);
				if(dtout==null)
				{
					this.SetErrorMsgPageBydir("查询出错，请重试！");
					btnExcel.Enabled=false;
        ...
        protected void Button4_Click(object sender, EventArgs e)
        {
            ...
            string strGoodsID = txtGoodsID.Text.Trim();
            string strGoodsName = txtGoodsName.Text.Trim();
            string strDeptId = ddlMD.SelectedValue;
            try
            {
                DataTable dtout = m1.GetGoodsDeptPrice(strGoodsID, strGoodsName,strDeptId);
                if (dtout == null)
                {
                    this.SetErrorMsgPageBydir("查询出错，请重试！");
                    btnExcel.Enabled = false;
